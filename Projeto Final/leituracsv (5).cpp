...
void shell_sort(dados* vet, int size) {
...
// Em seguida é feita a impressão do número de registros e de cada campo
{
    ...
    {
        cout << endl << "Preco:" << vetor[i].preco << endl;
        cout << "Codigio de barras:" << vetor[i].codigo_barras << endl;
...
// Se o identificador existir(for diferente de -1) é feito a alteração, através de uma subscrição do campo escolhido.
{
    ...
    if (posicao != -1)
    {
        ...
        if (buscaCampo == 3)
        {
            ...
            cin.ignore();
            cin.getline(vetor[posicao].descricao, tamanho);
        ...
        if (buscaCampo == 4)
        {
            ...
            cin.ignore();
            cin.getline(vetor[posicao].status, tamanho);
...
{
    int qtd = 0, tamanho = 0;
    importarcsv("base10.csv", "arqvsaida", tamanho);
    ...
    dados *medicamentos = new dados[tamanho];  //??????quantidade ordenar por pelo menos dois campos 
    int opcao;
    cout << endl;
